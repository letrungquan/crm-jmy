...
import { Customer, Sale, StatusConfig, Lead, CustomerData } from '../types';
...
const CustomerDetailView: React.FC<CustomerDetailViewProps> = ({ customer, sales, statuses, onClose, onSelectLead, onUpdateCustomer, onEdit, onDelete, onAddNote, currentUser }) => {
    ...
    const [relationship, setRelationship] = useState(customer.relationshipStatus || 'chot_don');
    ...
    useEffect(() => {
        setRelationship(customer.relationshipStatus || 'chot_don');
            ...
            <header className="pb-4 border-b border-slate-200">
                ...
                <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-3 grid grid-cols-2 md:grid-cols-5 gap-y-4 gap-x-2">
                    <div className="col-span-2 md:col-span-1">
                        ...
                        <select value={relationship} onChange={handleRelationshipChange} className="w-full text-sm font-bold bg-transparent text-green-900 focus:outline-none -ml-1">
                           ...
                           <div className="space-y-1">
                                ...
                                <div className="flex justify-between text-sm py-1.5">
                                    ...
                                    <span className="font-medium text-slate-700 text-right truncate">
                                        {customer.website ? <a href={customer.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{customer.website}</a> : '(Chưa có dữ liệu)'}
                                    </span>
                                </div>
                           </div>
                       </CollapsibleSection>
